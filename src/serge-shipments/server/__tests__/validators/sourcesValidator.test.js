...
const sourceEntry = params.req.dependencies.sergeSourcesUrl + "/v1/sources/123";
...
describe("sourcesValidator", () => {
  ...
  describe("for validate()", () => {
    ...
    it("throws an error if source does not exist", async () => {
      ...
      try {
        let result = await sourcesValidator(sourceEntry, params);
      } catch (err) {
    ...
    it("succeeds if source exists", async () => {
      ...
      await sourcesValidator(sourceEntry, params);
    });
  });
});
